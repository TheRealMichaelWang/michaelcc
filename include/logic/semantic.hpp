...
namespace michaelcc {
    ...
    private:
        ...
        private:
            ...
            void forward_declare_function(const std::string& function_name, const ast::ast_element& return_type, const std::vector<ast::function_parameter>& parameters, const uint8_t qualifiers, const source_location& location);
        public:
            ...
            void visit(const ast::function_declaration& node) override {
                forward_declare_function(node.function_name(), *node.return_type(), node.parameters(), node.qualifiers(), node.location());
            }
