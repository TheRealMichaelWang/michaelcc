...
namespace michaelcc {
	namespace logical_ir {
		...
		private:
			...
			bool m_is_global;
		public:
			variable(std::string&& name, std::unique_ptr<typing::type>&& var_type, bool is_global, std::weak_ptr<symbol_context>&& context)
				: symbol(std::move(name), std::move(context)), m_type(std::move(var_type)), m_is_global(is_global) {}

			const std::unique_ptr<typing::type> get_type() const noexcept { return m_type->clone(); }
		...
		private:
			uint64_t m_value;
			std::unique_ptr<typing::int_type> m_type;
		public:
			integer_constant(uint64_t value, std::unique_ptr<typing::int_type>&& int_type)
		...
		private:
			double m_value;
			std::unique_ptr<typing::float_type> m_type;
		public:
			floating_constant(double value, std::unique_ptr<typing::float_type>&& float_type)
		...
		private:
            ...
            std::unique_ptr<typing::type> m_result_type;
		public:
			binary_operation(token_type op,
				std::unique_ptr<expression>&& left,
				std::unique_ptr<expression>&& right,
				std::unique_ptr<typing::type>&& result_type)
				: m_operator(op),
		...
		private:
			std::unique_ptr<expression> m_operand;
			std::unique_ptr<typing::type> m_target_type;
		public:
			type_cast(std::unique_ptr<expression>&& operand, std::unique_ptr<typing::type>&& target_type)
		...
		private:
			...
			std::unique_ptr<typing::type> m_field_type;
		public:
			member_access(std::unique_ptr<expression>&& base, size_t field_index, std::unique_ptr<typing::type>&& field_type)
		...
		public:
			...
			const std::unique_ptr<typing::type> get_type() const override {
				...
				if (arr_type != nullptr) {
					return arr_type->element_type()->clone();
				}
		...
		public:
			...
			const std::unique_ptr<typing::type> get_type() const override {
				...
				if (ptr_type != nullptr) {
					return ptr_type->pointee_type()->clone();
				}
