...
namespace michaelcc {
	namespace ast {
		...
		public:
			explicit int_literal(uint8_t qualifiers, int_class m_class, size_t value)
		...
		public:
			explicit float_literal(float value)
				: m_value(value) { }
		...
		class double_literal : public lvalue {
		private:
			double m_value;

		public:
			explicit double_literal(double value)
				: m_value(value) { }
		...
		private:
			...
			std::unique_ptr<rvalue> m_index;

		public:
			get_index(std::unique_ptr<lvalue>&& ptr, std::unique_ptr<rvalue>&& index)
				: m_ptr(std::move(ptr)),
